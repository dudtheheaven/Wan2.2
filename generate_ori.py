...
import time #scy modi
import json #scy modi
...
def generate(args):
    ...
    if rank == 0:
        ...
        result = {
            ...
            "elapsed_sec": elapsed if "t2v" in args.task else None,
            "peak_alloc_gb": peak_alloc if "t2v" in args.task else None,
            "peak_reserved_gb": peak_reserved if "t2v" in args.task else None,
            "prompt": args.prompt,
        }
        ...
        try:
            ...
            result["gpu"] = ", ".join([f"{k}x{v}" for k, v in c.items()]) if c else None
        except Exception:
            result["gpu"] = None

        # 로그에 1줄 JSON으로 남김 (파싱 쉬움)
        logging.info("RUN_METRICS_JSON=" + json.dumps(result, ensure_ascii=False))

        # 파일로도 저장하고 싶으면(추천): mp4 파일명과 같은 stem으로 저장
        metrics_path = os.path.splitext(args.save_file)[0] + ".metrics.json"
        with open(metrics_path, "w", encoding="utf-8") as f:
            json.dump(result, f, ensure_ascii=False, indent=2)

        # ===== [메타/측정 결과 출력/저장 끝] ===== scy modi
